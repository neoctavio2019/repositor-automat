...
Given('Abro el portal password', { timeout: 60 * 1000 }, function() {
    return this.driver.get('http://localhost/');
});


Then('Hago click en input', { timeout: 60 * 1000 }, function() {
    return this.driver.findElement({ id: "lstOpcionesBusqueda" }).click();
});
Then('Selecciono length', { timeout: 60 * 1000 }, function() {
    return this.driver.findElement({ xpath: "/html/body/app-root/div/div/div/div[1]/input" }).click();
});
Then('Ingreso length {int}', { timeout: 60 * 1000 }, function(int2) {
    return this.driver.findElement({ xpath: "/html/body/app-root/div/div/div/div[1]/input" }).sendKeys(int2);
});

Then('Selec uso letras', { timeout: 60 * 1000 }, function() {
    return this.driver.findElement({ xpath: "/html/body/app-root/div/div/div/div[2]/div/label/input" }).click();
});

Then('Selec uso números', { timeout: 60 * 1000 }, function() {
    return this.driver.findElement({ xpath: "/html/body/app-root/div/div/div/div[3]/div/label/input" }).click();
});

Then('Selec uso símbolos', { timeout: 60 * 1000 }, function() {
    return this.driver.findElement({ xpath: "/html/body/app-root/div/div/div/div[4]/div/label/input" }).click();
});

Then('Presiona generar', { timeout: 60 * 1000 }, function() {
    return this.driver.findElement({ xpath: "/html/body/app-root/div/div/div/button" }).click();
});
